...
describe("aaveActionUtil", () => {
  ...
  describe("L1 sepolia with native wallet", () => {
    ...
    beforeAll(async () => {
      await approve(
        evmWalletProvider,
        markets.AaveV3Sepolia.ASSETS.USDC.UNDERLYING,
        market.POOL,
        ALLOWANCE_DEFAULT,
      );
    });
    ...
    test("supply and withdraw USDC work with native wallet", async () => {
      ...
      await supplyAndWithdraw(
        evmWalletProvider,
        provider,
        { user, amount, market, asset },
        market.POOL,
      );
    });
    ...
    test("supply and withdraw ETH work with native wallet", async () => {
      ...
      await supplyAndWithdraw(
        evmWalletProvider,
        provider,
        { user, amount: 1n, market, asset },
        market.WETH_GATEWAY,
      );
    });
  });

  describe.each([["viem"], ["cdp"]])("L2 base sepolia with evm provider %s", walletType => {
